...
namespace B2BAISERA.Models.EFServer
{
    public partial class CUSTOM_DOWNLOAD_TRANSACTIONDATA
    {
        ...
        public virtual ICollection<CUSTOM_S02002> CUSTOM_S02002
        {
            get
            {
                if (_cUSTOM_S02002 == null)
                {
                    var newCollection = new FixupCollection<CUSTOM_S02002>();
                    newCollection.CollectionChanged += FixupCUSTOM_S02002;
                    _cUSTOM_S02002 = newCollection;
                }
                return _cUSTOM_S02002;
            }
            set
            {
                if (!ReferenceEquals(_cUSTOM_S02002, value))
                {
                    var previousValue = _cUSTOM_S02002 as FixupCollection<CUSTOM_S02002>;
                    if (previousValue != null)
                    {
                        previousValue.CollectionChanged -= FixupCUSTOM_S02002;
                    }
                    _cUSTOM_S02002 = value;
                    var newValue = value as FixupCollection<CUSTOM_S02002>;
                    if (newValue != null)
                    {
                        newValue.CollectionChanged += FixupCUSTOM_S02002;
                    }
                }
            }
        }
        private ICollection<CUSTOM_S02002> _cUSTOM_S02002;
        ...
        private void FixupCUSTOM_S02002(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.NewItems != null)
            {
                foreach (CUSTOM_S02002 item in e.NewItems)
                {
                    item.CUSTOM_DOWNLOAD_TRANSACTIONDATA = this;
                }
            }
            ...
            if (e.OldItems != null)
            {
                foreach (CUSTOM_S02002 item in e.OldItems)
                {
                    if (ReferenceEquals(item.CUSTOM_DOWNLOAD_TRANSACTIONDATA, this))
                    {
                        item.CUSTOM_DOWNLOAD_TRANSACTIONDATA = null;
